        ...
        methods: {
            setClient: function setClient(target) {
                ...
                if (this.btnName == "Connect" || target == 'check') {
                    ...
                    enable = true;
                    this.$store.dispatch("call", { api: 'internetreachable', async: true }).then(function (result) {
                        if (!result.reachable) {
                            that.$message({
                                type: 'warning',
                                msg: -210
            ...
            rmAll: function rmAll() {
                var that = this;
                that.rmStatus = true;
                that.$store.dispatch("call", { api: 'ovpnrmall' }).then(function (result) {
                    that.rmStatus = false;
                    if (result.failed) {
                        that.$message({
                            type: "error",
            ...
            modifyClient: function modifyClient(index) {
                var that = this;
                ...
                if (description != null && description.length == 0) {
                    ...
                    this.$store.dispatch("call", {
                        api: "ovpnmodifyclient", data: data
                    }).then(function (result) {
                        if (result.failed) {
                            that.$message({
                                "type": "error",
            ...
            onupload: function onupload(ajaxData) {
                var that = this;
                this.sta = "uploading";
                this.$store.dispatch("call", { api: "ovpnuploadconfigpack", data: ajaxData, timeOut: 120000 }).then(function (result) {
                    if (result.success) {
                        that.vpnState = false;
                        that.resetSta();
                        that.verifyFirmware();
            ...
            addVpnClient: function addVpnClient() {
                ...
                that.$store.dispatch("call", {
                    api: "ovpngenerateclient", data: data, timeOut: 120000
                }).then(function (result) {
                    that.closeModal("add");
                    if (result.failed) {
                        that.$message({
            ...
            verifyFirmware: function verifyFirmware() {
                var that = this;
                this.$store.dispatch("call", { api: "ovpncheckconfigpack", timeOut: 30000 }).then(function (result) {
                    // console.log(result);
                    if (result.failed) {
                        that.vpnNumHtml = 0;
                        that.$message({
            ...
            getData: function getData() {
                var that = this;
                that.clients = [];
                this.$store.dispatch("call", { api: "ovpnGetClients" }).then(function (result) {
                    if (result.failed) {
                        that.$message({
                            "type": "error",
                            "api": "ovpnGetClients",
                    ...
                    if (result.success) {
                        ...
                        that.$store.dispatch("call", { api: "ovpngetclientstatus" }).then(function (result) {
                            clearInterval(that.timer);
                            that.timer = setInterval(function () {
                                that.$store.dispatch("call", { api: "ovpngetclientstatus" });
                            }, 5000);
